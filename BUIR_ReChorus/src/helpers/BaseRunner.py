...
class BaseRunner(object):
	...
	def predict(self, dataset: BaseModel.Dataset, save_prediction: bool = False) -> np.ndarray:
		...
		# Check for shape inconsistency in predictions
		prediction_shapes = [p.shape for p in predictions]
		if len(set(prediction_shapes)) > 1:
			logging.warning(f"Inconsistent shapes detected: {prediction_shapes}")
			# Optionally, pad or truncate predictions to a fixed shape
			max_shape = max(prediction_shapes, key=lambda x: x[0])[0]  # Get the max length
			predictions = [np.pad(p, ((0, max_shape - p.shape[0]), (0, 0)), mode='constant', constant_values=-np.inf)
						   if p.shape[0] < max_shape else p for p in predictions]

		predictions = np.array(predictions)
		predictions = predictions.reshape(predictions.shape[0], -1)  # (14681, 256 * 100)
