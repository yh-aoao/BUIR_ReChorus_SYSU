...
class LGCN_Encoder(nn.Module):
    def __init__(self, user_count, item_count, latent_size, edge_index, n_layers=3, drop_flag=False):
        ...
        # 添加注意力机制
        self.attention_weights = nn.Parameter(torch.Tensor(n_layers, latent_size))
    ...
    def forward(self, inputs):
        ...
        for i, conv in enumerate(self.convs):
            ...
            # 计算注意力分数
            score = (x * self.attention_weights[i]).sum(dim=-1)
        ...
        # 加权求和得到最终的嵌入
        final_embeddings = (all_embeddings * attention_weights.unsqueeze(-1)).sum(dim=1)

        user_all_embeddings = final_embeddings[:self.user_count, :]
        item_all_embeddings = final_embeddings[self.user_count:, :]
